...
export default function LogbookTab({
  ...
  // Convert Google Sheets URL to safe embed preview URL
  ...
  }, [reportList, statusFilter, searchQuery]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white p-3.5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-wrap items-center gap-2">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider px-2 flex items-center gap-1.5">
              <Layers className="w-3.5 h-3.5 text-blue-600" /> Pilih Form Logbook ({logbookCategories.length}):
            </span>
      ...
      {/* ========================================================= */}
      {/* TAB 2: RIWAYAT & REKAP LOG (GOOGLE SHEETS)                */}
      {/* ========================================================= */}
      {logbookMainTab === 'rekap' && (
        <div className="space-y-6">
          ...
          {/* 2. TAMPILAN SPREADSHEET LIVE GOOGLE SHEETS */}
          <div className="bg-white rounded-2xl shadow-xs border border-slate-200 overflow-hidden relative min-h-[750px]">
            <div className="px-4 py-2.5 bg-slate-50 border-b border-slate-200 flex items-center justify-between text-xs">
              <span className="font-semibold text-slate-700 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                Tampilan Live Google Sheets: <span className="font-bold text-slate-900">{currentFormObj.title}</span>
              </span>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-emerald-700 hover:text-emerald-900 font-bold flex items-center gap-1 ml-2"
                >
                  Buka Layar Penuh <ExternalLink className="w-3 h-3" />
                </a>
              </div>
            ...
            {isSheetIframeLoading && (
              <div className="absolute inset-0 bg-white/90 z-10 flex flex-col items-center justify-center space-y-3">
                ...
                <p className="text-sm font-bold text-slate-700">Memuat spreadsheet {currentFormObj.title}...</p>
              </div>
            )}
            <iframe
          ...
          {/* 3. LOG OPERASIONAL TERDAFTAR (STANDAR TABEL) */}
          <div className="bg-white rounded-2xl shadow-xs border border-slate-200 overflow-hidden">
            <div className="p-4 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">Daftar rekaman logbook operasional terverifikasi.</p>
              </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-8 pr-3 py-1.5 text-xs bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 w-44 sm:w-52"
                  />
                </div>
                ...
                <select
                  ...
                  className="px-3 py-1.5 bg-white border border-slate-200 rounded-xl text-xs font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
                >
                ...
                {currentUser?.role === 'ADMIN' && selectedReportIds.length > 0 && (
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-xl text-xs font-bold shadow-xs transition-all cursor-pointer"
                  >
                    <Trash2 className="w-3.5 h-3.5" /> Hapus ({selectedReportIds.length})
                  </button>
                )}

                <button
                  onClick={() => window.print()}
                  className="flex items-center gap-1.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer"
                >
              ...
              <table className="w-full text-left border-collapse text-xs whitespace-nowrap">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  ...
                  {filteredReports.length === 0 && (
                    <tr>
                      <td colSpan={currentUser?.role === 'ADMIN' ? 9 : 8} className="p-8 text-center text-slate-400">
                        Tidak ada catatan log operasional yang cocok dengan pencarian / filter.
                      </td>
                    </tr>
                  )}
                </tbody>
